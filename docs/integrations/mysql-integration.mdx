...
```
<Note>You can skip the fields `username` and `password`, if your database doesn't require authentication. If you skip `database`, then this integration will collect metrics from all databases, else It will collect for mentioned database only.</Note>
