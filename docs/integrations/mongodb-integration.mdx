...
### Note for Docker Agent Users
If you are running the Middleware Docker agent, then we recommend to keep the creds YAML under `/var/log` directory. 

This is because, the Docker agent will not allow dynamic volume binding.
