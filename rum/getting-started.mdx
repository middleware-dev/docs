...
<Accordion title="Script Integration Issues">
    If you are experiencing exceptions and errors while integrating the RUM browser SDK, consider moving Middleware packages and scripts earlier in the execution of your application code to avoid collisions with your website's existing code.
...
<Accordion title="Unhandled Promise Rejection: TypeError: Failed to fetch">
    This error can be ignored, as it does not impact the Middleware RUM Browser SDK. This issues commonly occurs when an API call is being executed and a user refreshes the page. 
</Accordion>
