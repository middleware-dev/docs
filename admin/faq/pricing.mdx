...
<Accordion title="How can I be sure my data is secured?">
    We keep security as our top priority including universal HTTPS, strong TLS configurations, and HTTP strict transport security. Middleware is compliant with Data security protocols such as GDPR, CCPA, HIPPA, and SOC 2 type ||. Read more about [security policies](https://middleware.io/security).
</Accordion>

<Accordion title="What are the billing options?">
...
<Accordion title="Can I subscribe only for one product (ex: log monitoring) and not for others (ex: APM)?">
...
<Accordion title="Can I run apm with container?">
...
<Accordion title="How can I manage high log volume in cost effecting way?">
...
<Accordion title="What happens if I get unexpectedly high number of hosts for some time?">
...
<Accordion title="How charges works in serverless monitoring?">
    For 1M traces generated from serverless application , you will be charged $5. 
</Accordion>

<Accordion title="How many events are included per host?">
    In infrastructure monitoring , you can monitor unlimited events(metrics) per host without any extra cost. 
</Accordion>

<Accordion title="How charges are calculated in synthetic monitoring?">
...
<Accordion title="Do you have a discount for startups and nonprofits?">
