...
Add tags by including the `MW_HOST_TAGS` environment variable to the installation command. Include any desired tags as comma-separated key-value pairs, as shown below. Use the tag `name` to create an alias for the host (e.g. production). 
