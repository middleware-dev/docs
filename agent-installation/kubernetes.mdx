...
If you are using another thirdy-party resource for mananging your Kubernetes Infrastructure (e.g. Lens), you must create cluster names that match the names of the contexts used by that third party resource. 

Change the `cluster.name` attribute by renaming the `MW_KUBE_CLUSTER_NAME` environment variable to the name(s) you are using in your thrid party resource. Once the cluster is renamed, it will automatically reset and start to catch the appropriately labeled environment variables.
</Accordion>
