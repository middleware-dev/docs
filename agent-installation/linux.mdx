...
To change the MW Agent internal metrics port, use the `MW_AGENT_INTERNAL_METRICS_PORT` environment variable. Use the following example by replacing the port number (which is by default set to `8888`):

```bash Shell
MW_AGENT_INTERNAL_METRICS_PORT=8888 MW_API_KEY=upnycrphsnoeliwoiyzpnmkqxfihohbbkuth MW_TARGET=https://xejkc_mdt.middleware.io:443 bash -c "$(curl -L https://install.middleware.io/scripts/rpm-install.sh)"
```
