---
title: May 8, 2024
---

# Features

- __Settings__ -> [Log Ingestion Controls for Kubernetes](https://docs.middleware.io/admin/settings/advanced) to limit Kubernetes log collection
![](https://lh7-us.googleusercontent.com/xhYdbF6wdb-YM_DmNn2A-MnRPXS5bIlSuSLdO2uvFkTVy-6UDHOD3-bwvZDzLH_QmqrbUcnM5M5Xpo_rfk0RbHuLGJsIvISdBR2qD8aca-qg43SW9ooI_xTGjSXSmjgdP1NtnM6uJ_B2_Y1iFmH386Y)
- Added option for users to get notified when alert status changes from `any status` to `ok`
- Added "max log size" ingestion control for logs
- Made more log metrics in the Apache Integration available for collection
- Added the following RPM Package Managers to the Linux Installation:
    - __Rocky Limnux__
    - __AlmaLinux__
    - __SUSE linux__
    - __Oracle Linux__

# Changes

- Added `diff()` function at the alert configuration level to only display applicable data types
- Added search feature that allows users to search logs by clicking on individual attribute value
- Users can now expand attribute values in JSON format

# Fixes

- Fixed filter attributes load buffering issue
- Fixed alert pagination issue
- Fixed alert issue to ensure users are notified in real-time when the alert status changes to `resolved`
- Fixed Custom Metrics logging issue
- 


