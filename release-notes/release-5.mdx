...
- Dashboards can be dynamically generated based on a single metric
- Filter the Span List within the APM Dashboard by parent `SpanID`
- Added __Critical__ severity level message to be auto-detected across all logs

# Changes

- All users will get the benefits of the Free Tier every month, even if they are on a paid plan
- The Free Tier will be reset every month for all customers across all products; this feature is live for new and existing users
...
- Dashboard Builder has a fully revamped dashboard and widget creation workflow
...
- Global Sidebar displays the filter value count for each selected item 
- Added screen pagination in the Synthetic Monitoring page
...
- Fixed `hasError` attribute tag across the entire application when users go through `total errors` from the dashboard to any other modules
