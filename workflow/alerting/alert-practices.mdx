...
The following alert tracks the average CPU Utilization for a Kubernetes node. This alert is monitoring a single Kubernetes node, specified by the Filter configuration. A Warning alert triggers whenever the average CPU Utilization surpasses three CPUs on the specified Kubernetes node, while a Critical alert activates when the average usage reaches five CPUs. This is checked on a five minute interval.
