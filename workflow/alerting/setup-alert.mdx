...
<Note> For more information on metrics collected in your environment, see the [Metrics Control Panel](https://middleware.io/settings/metrics-control-panel).</Note>
...
A trigger is a __Warning__ or __Critical__ message that is sent to users when the data source a user has selected exceeds a pre-defined threshold. The window indicates the interval by which these trigger conditions are checked. 
