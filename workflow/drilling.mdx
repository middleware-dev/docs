---
title: Drilling Into Your Data
---

The Drill Down feature enhances your ability to investigate, analyze, and resolve issues without needing to switch contexts. Instead of sifting through large volumes of data indiscriminately, focus your efforts on drilling into relevant and focused subsets of data in order to save your team time and effort.

Clicking on any of Middleware's interactive tables will produce popouts that connect you to all information related to that data point that Middleware has collected. These popouts will allow you to drill down to the root cause regardless of which data point you start from.

# Metrics

Metrics are compact statistical representations of system state and resource utilization. They are quantifiable data points within your software system, like throughput, resource utilization, and error rates.

Metrics provide a high-level view of the system in question, whereas traces and logs provide you with a more granular look into your system-level data. Access metrics by drilling into the Logs page:

![](https://lh7-us.googleusercontent.com/IptT7ke9yQwuDqwr9W5hGBU88OotciscteZKEQQq_9HHRJSBGczQ7hrsJrK4p8KFHL0eFch4dlL_RutGikzlyuQVU96zkKqKKS1KNySqXdbLMKZcT35Z5cf5s_1YH7vGertVlM8PvDC9Y670r4a_LMc)

![](https://lh7-us.googleusercontent.com/CHXQUNx1dEHyGZ6kwg-ZijrdF6S3uU0jpHfvFhioCybgF9X8T-duGVCt94xgFxXZ2pZYXDwnnJPRiwu7xnmdIj_9RntqjaYZgmhUVw8tMNY7e-Oe1PZHAsDneV2UTrTsvhhV9SXSmWWVbdKkzHZUbkA)

# Events

Events are discrete occurrences that transpire at a particular moment in time. They can be as broad as an uncaught exception, or specific like a user logout or button click. 

Access events by drilling into the APM page from the Span List:

![](https://lh7-us.googleusercontent.com/jIdByPn4wQ0PPMPqSLy78UazSKT7bm48a1s6rO53lI8LUfpYSJdb-y2ofG6U37wGEneFcbXlljec7QVCm2RrcJn_hG4ssptW0myuSUHl9t0ZyEfIbNBHG_ril3QGuTuSdFar9Tm-dMrFKtREdX4HKB8)

![](https://lh7-us.googleusercontent.com/D0s7GT2KiqaByJ6HJmA3AsSR8LbWLQcsyCMBtZ20tt2uiN-k1f9Pv6CsFJk6V5-5hXUeUdXnm_BZQ_vidTCHv2gAzGTNy8_0foSj4HMAzR3QNGeS5gd3gTSFpkawMFYZO0nD9T1-toG6JTtqNPCavBE)

# Spans

<<<<<<< HEAD
Spans are structured log messages that represent a single operation in a trace. They allow you to break down your traces into discrete operations. 

Multiple connected spans form a trace, with a single root span that describes the entire operation. Spans are useful for identifying system performance bottlenecks and dependencies across your infrastructure in a concise, structured, and human-readable format. Access spans by drilling into the APM Page:

![](https://lh7-us.googleusercontent.com/vXXeBkRy_KuV7-i0gDlUmkhZ_faKLga2nhawmDG9PzuEf_DlhSgfW1qznl9UWCe82yl9HDY-eyXmbEJUjim9Kz-1_CWaf8QWLIwplXDPOpBhDLsDUXz9-6BNtPAcBvLmbucDXmzkgjeUBDmtjMH0ARw)
=======
Spans are structured log messages that represent a single operation in a trace. They allow you to break down your traces into discrete operations. Multiple connected spans form a trace, with a single root span that describes the entire operation. 

Spans are useful for identifying system performance bottlenecks and dependencies across your infrastructure in a concise, structured, and human-readable format.
>>>>>>> e3c963939f2b53db5ac7bb21d027019b454042c0

# Traces

Traces are the entire path of a transaction or request as they flow through various components of your distributed system. Traces are particularly useful for identifying the root cause of a seemingly isolated issue in a broader context, such as latency cascading and dependency failures. 

Distributed Traces look at entire operations and how they form transactions. Distributed Traces are composed of spans, with a start and end time and location where they occur. Access traces by drilling into the APM or Logs Page:

<Note> The following is an example of drilling into your traces from the APM section. </Note>

![](https://lh7-us.googleusercontent.com/7A28mSCxMZzMwbzW57928pxHTUVFgJWFT7ACrbn0sIufBq4KpY3F0Kx7TrvmEdgqe64VcxT7fc2eiYlZ29UCHx1u7tEz6DgIgsy6W257VRkXqF8EHX4Q0_OsBBqYhdQU2GL2xnqx3s-0RgZynebs2Ag)

![](https://lh7-us.googleusercontent.com/zL8CeS9WH5kFM8YNEKE8K6hEmRw578Y-x1JD7W9SAGT4pkqALJ3x9ZbM6iOltmZas5jV_FQaKtFMYXAtU_Sb2vFPTA7m6-HRnPzK4yCC8A0-9M45tVzTukvjslBilYg5Ll96jNfRJDiy38xNcNjnsHw)

![](https://lh7-us.googleusercontent.com/cr4jFifb2C5deMM-LfakE1WNGbHy8SZFxAIMy19ArwVqddripfHfhtUzU5Ke6vbpnPU3_hHkw9MJ09sfcfkqtGhYPX_-ZwmpFrpR9x1Nl60KlLrzF_yEloVl9jbNfUF1ZDS8piYEVnF-DRPjF_yiFy8)

### Trace Attributes

Trace Attributes are key-value pairs that contain metadata associated with the span you are investigating.

### Error Traces

<<<<<<< HEAD
Error traces are records of the sequence of function calls and their associated parameters and variables that occur when an error or exception occurs in your software application. They are useful for correlating other types of data collected by your observability product, such as performance metrics, log messages, or system events. 
=======
Error traces are records of the sequence of function calls and their associated parameters and variables that occur when an error or exception occurs in your software application. 

Error Traces are useful for correlating other types of data collected by your observability product, such as performance metrics, log messages or system events. 

### Access Traces from Logs
>>>>>>> e3c963939f2b53db5ac7bb21d027019b454042c0

# Logs

Logs are timestamped records of text that contain information about events, activities, and messages generated by your distributed software architecture. Logs contain metadata that can be structured or unstructured. 
<<<<<<< HEAD
=======

Logs are at the core of most drill down practices and are useful when drilling into resolving event-based issues, post-mortem analysis related to outages, and monitoring complex systems patterns and potentially abnormal behavior.
>>>>>>> e3c963939f2b53db5ac7bb21d027019b454042c0

Logs are at the core of most drill-down practices and are useful when drilling into resolving event-based issues, post-mortem analysis related to outages, and monitoring complex systems patterns and potentially abnormal behavior. Access logs by drilling into the Logs Page:

<Note> For more information on monitoring logs, navigate to [Log Monitoring Overview](https://docs.middleware.io/workflow/log-monitoring/overview). </Note>

![](https://lh7-us.googleusercontent.com/xiCBl0_LaVivm5MqRMHwixoSmoP0FPMKIaPFdufvVeLYhxKS2wA6sqqrfmLCn0kG5Q2Ae2k2hZMfZCKRBm45kMPOi98Gaq8-ROVMDTI2b874aghErxA7p8EvsYQDuu5yYpK6oFq0XG_wvMfW_ItalQk)

# Next Steps

- [Log Monitoring Overview](https://docs.middleware.io/workflow/log-monitoring/overview)
- [Infrastructure Monitoring](https://docs.middleware.io/workflow/infrastructure-monitoring/container-monitoring)
- [Intro to APM](https://docs.middleware.io/workflow/apm/apm-overview)
- [Real User Monitoring (RUM)](https://docs.middleware.io/rum/rum-overview)
- [Continuous Profiling](https://docs.middleware.io/workflow/apm/continuous-profiling)

<Note> Need assistance or want to learn more about Middleware? Contact us at support[at]middleware.io. </Note>