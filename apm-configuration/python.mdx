...
Add the following to your `main()` function in `manage.py` if you are using Django:

```python3 Python
tracker.django_instrument()
```

#### Step 5b: Initialize Your Django Project [Optional]

After deploying your application, run the following command:

```python Python
MIDDLEWARE_CONFIG_FILE=./middleware.ini middleware-apm run python app.py
```

#### Step 5c: Start Your Django Project [Optional]
